...
with open('frontend/src/portals/FacultyPortal.jsx', 'r', encoding='utf-8') as f:
    code = f.read()
...
'''
code = code.replace('// ── FACULTY ANALYTICS', row_component + '\n// ── FACULTY ANALYTICS')

# 3. Replace table body for FacultyAnalytics
code = re.sub(
    r'\{myCourses\.map\(\(c,\s*i\)\s*=>\s*\{\s*const att = \[89,\s*72,\s*95\]\[i % 3\];\s*const avgGrade = \[82,\s*77,\s*91\]\[i % 3\];\s*return \([\s\S]*?\);\s*\}\)\}',
...
old_pin_div = '''<div style={{ fontSize: 42, fontWeight: 700, fontFamily: 'var(--font-mono)', letterSpacing: '4px', color: 'var(--text-1)', lineHeight: 1 }}>
                    742819
                  </div>'''
new_pin_div = '''<div style={{ fontSize: 32, fontWeight: 700, fontFamily: 'var(--font-mono)', letterSpacing: '2px', color: 'var(--text-1)', lineHeight: 1 }}>
                    {sessionData.sessionId?.split("-")[0].toUpperCase()}
                  </div>'''
code = code.replace(old_pin_div, new_pin_div)

code = re.sub(
    r'<div style=\{\{\s*width:\s*\'100%\',\s*height:\s*\'100%\',\s*background:\s*\'#000\',\s*borderRadius:\s*4,\s*display:\s*\'grid\',\s*gridTemplateColumns:\s*\'repeat\(5,\s*1fr\)\',\s*gap:\s*2,\s*padding:\s*4\s*\}\}>[\s\S]*?</div>',
    '<img src={sessionData.qrBase64} alt="QR Code" style={{ width: "100%", height: "100%", objectFit: "contain" }} />',
    code
)

with open('frontend/src/portals/FacultyPortal.jsx', 'w', encoding='utf-8') as f:
    f.write(code)
